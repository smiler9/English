...
│  ├─ level08~20/index.ts   # Level 8~20 생성 콘텐츠 (15~27유닛)
│  ├─ generatedAdvancedContent.ts # Level 8~20 유닛/레슨 생성 엔진
│  ├─ generatedScenarios.ts # Level 8~20 회화 시나리오 생성
...
- **20단계 구조는 미리, 콘텐츠는 전 단계 연결**: `levels.ts`엔 20개 다 있고 `units`는 Level 1~20까지 채워짐.
...
- **Level 8~20은 생성 기반**: 레벨별 문법 패턴과 상황 주제를 조합해 유닛/레슨/시나리오를 자동 생성하고, 이후 필요한 레벨부터 사람이 보강하는 흐름.
...
## 다음 할 일
- Level 8~20 생성 콘텐츠를 실제 학습 품질 기준으로 순차 보강
